 ...
 * </p>
 * <ul>
 * <li>MediaPicture: {@link MediaPicture} objects represent uncompressed video
 * in Humble.</li>
 * <li>Timestamps: All {@link Media} objects in Humble have a timestamp, and
 * this demonstration introduces the concept of having to worry about
 * <i>when</i> to display information.</li>
 * </ul>
 * 
 * <p>
...
public class DecodeAndPlayVideo {
	 ...
	 */
	private static void playVideo(String filename) throws InterruptedException,
			IOException {
		/*
		 * Start by creating a container object, in this case a demuxer since we
		 * are reading, to get video data from.
		 */
		...
		/**
		 * This is the Window we will display in. See the code for this if
		 * you're curious, but to keep this demo clean we're 'simplifying' Java
		 * AWT UI updating code. This method just creates a single window on the
		 * UI thread, and blocks until it is displayed.
		 */
		...
		/**
		 * Media playback, like comedy, is all about timing. Here we're going to
		 * introduce <b>very very basic</b> timing. This code is deliberately
		 * kept simple (i.e. doesn't worry about A/V drift, garbage collection
		 * pause time, etc.) because that will quickly make things more
		 * complicated.
		 * 
		 * But the basic idea is there are two clocks:
		 * <ul>
		 * <li>Player Clock: The time that the player sees (relative to the
		 * system clock).</li>
		 * <li>Stream Clock: Each stream has its own clock, and the ticks are
		 * measured in units of time-bases</li>
		 * </ul>
		 * 
		 * And we need to convert between the two units of time. Each
		 * MediaPicture and MediaAudio object have associated time stamps, and
		 * much of the complexity in video players goes into making sure the
		 * right picture (or sound) is seen (or heard) at the right time. This
		 * is actually very tricky and many folks get it wrong -- watch enough
		 * Netflix and you'll see what I mean -- audio and video slightly out of
		 * sync. But for this demo, we're erring for 'simplicity' of code, not
		 * correctness. It is beyond the scope of this demo to make a full
		 * fledged video player.
		 */
		...
		// similar with the demuxer, for the windowing system, clean up after
		// yourself.
		window.dispose();
	}
	...
	private static BufferedImage displayVideoAtCorrectTime(
			long streamStartTime, final MediaPicture picture,
	 ...
	 * a window and plays back the video.
	 * 
	 * @param args
